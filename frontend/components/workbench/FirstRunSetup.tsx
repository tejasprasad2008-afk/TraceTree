...
type ProviderId = "openai" | "claude" | "openrouter";

const PROVIDERS: { id: ProviderId; name: string; detail: string; keyLabel: string; baseUrl: string; modelHint: string }[] = [
  {
    id: "openai",
    name: "OpenAI",
...
export function FirstRunSetup({ apiKey, onComplete }: FirstRunSetupProps) {
  ...
  const save = async () => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:8000/api/setup/llm", {
        method: "POST",
    ...
    <main style={{ minHeight: "100vh", display: "grid", placeItems: "center", padding: 24, background: "radial-gradient(circle at 12% 15%, #263d4d 0, transparent 32%), var(--bg-app)" }}>
      <section style={{ width: "min(720px, 100%)", border: "1px solid var(--border-strong)", background: "var(--bg-panel)", padding: 28 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 30 }}>
          ...
          <div style={{ color: "var(--text-muted)", fontFamily: "var(--font-mono)", fontSize: 11 }}>{step < 4 ? `STEP ${step + 1} / 4` : "READY"}</div>
        </div>
        {step < 4 && <div style={{ display: "flex", gap: 4, marginBottom: 28 }}>{[0, 1, 2, 3].map((item) => <span key={item} style={{ height: 3, flex: 1, background: item <= step ? "var(--accent)" : "var(--border)" }} />)}</div>}
        <h2 style={{ margin: "0 0 8px", fontSize: 18 }}>{title}</h2>
        {step === 0 && <><p style={{ color: "var(--text-muted)", marginTop: 0 }}>Select the service that will generate optional AI triage summaries. OpenAI is first; you can change this later in your local <code>.env</code>.</p><div style={{ display: "grid", gap: 8 }}>{PROVIDERS.map((item) => <button key={item.id} onClick={() => { setProvider(item.id); setBaseUrl(item.baseUrl); }} style={{ textAlign: "left", padding: 14, border: `1px solid ${provider === item.id ? "var(--accent)" : "var(--border)"}`, background: provider === item.id ? "#273a42" : "var(--bg-inset)", color: "var(--text-primary)", cursor: "pointer" }}><strong>{item.name}</strong><span style={{ display: "block", marginTop: 3, fontSize: 11, color: "var(--text-muted)" }}>{item.detail}</span></button>)}</div></>}
        ...
        {step === 2 && <><p style={{ color: "var(--text-muted)", marginTop: 0 }}>The official default is prefilled. Press Continue to accept it, or paste a compatible endpoint.</p><label style={labelStyle}>Base URL</label><input autoFocus value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} style={inputStyle} /></>}
        {step === 3 && <><p style={{ color: "var(--text-muted)", marginTop: 0 }}>Enter the exact model identifier available to your account. TraceTree does not silently choose a different model.</p><label style={labelStyle}>Model name</label><input autoFocus value={model} onChange={(event) => setModel(event.target.value)} placeholder={selected.modelHint} style={inputStyle} /></>}
        {step === 4 && <div style={{ padding: "14px 0" }}><div style={{ color: "var(--sev-safe)", fontSize: 18, fontWeight: 700 }}>Configuration saved locally.</div><p style={{ color: "var(--text-muted)", maxWidth: 520 }}>Restart <code>cascade-analyze dashboard</code> once so the orchestrator loads your selected provider. Until then, this dashboard stays in safe local mock mode.</p></div>}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", gap: 10, marginTop: 30 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>{step > 0 && step < 4 && <button onClick={() => setStep(step - 1)} style={secondaryStyle}>Back</button>}{step === 0 && <button onClick={() => setStep(1)} style={primaryStyle}>Continue</button>}{step === 1 && <button disabled={!apiToken.trim()} onClick={() => setStep(2)} style={primaryStyle}>Continue</button>}{step === 2 && <button disabled={!baseUrl.trim()} onClick={() => setStep(3)} style={primaryStyle}>Continue</button>}{step === 3 && <button disabled={saving} onClick={save} style={primaryStyle}>{saving ? "Saving…" : "Save configuration"}</button>}{step === 4 && <button onClick={onComplete} style={primaryStyle}>Open Workbench</button>}</div>
        </div>
      </section>
    </main>
  );
