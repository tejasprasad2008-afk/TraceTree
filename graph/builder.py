...
def build_cascade_graph(
    ...
    for child_pid, parent_pid in parent_map.items():
        if parent_pid in G and child_pid in G:
            G.add_edge(
                parent_pid, child_pid, label="clone", severity=1.0, signature_tags=[]
            )
    ...
    for evt_idx, event in enumerate(events):
        ...
        if pid not in G:
            G.add_node(
                pid, label=f"PID:{pid}", type="process", severity=0.0, signature_tags=[]
            )
        ...
        if evt_type == "connect":
            ...
            G.add_edge(
                pid, node_id, label="connect", severity=risk, signature_tags=tags
            )

        elif evt_type == "openat":
            node_id = f"file_{target}"
            sensitive = (
                event.get("details", {}).get("sensitive", False)
            ...
            G.add_edge(
                pid, node_id, label=evt_type, severity=severity, signature_tags=tags
            )
    ...
    graph_stats = {
        ...
        "total_severity": round(
            sum(d.get("severity", 0.0) for _, _, d in G.edges(data=True)), 2
        ),
        "max_severity": round(
            max(
                (d.get("severity", 0.0) for _, _, d in G.edges(data=True)), default=0.0
            ),
            2,
        ),
        ...
        "avg_betweenness": round(
            sum(betweenness.values()) / len(betweenness) if betweenness else 0.0, 4
        ),
